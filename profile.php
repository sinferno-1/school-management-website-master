...
<body>
    <div class="container-fluid p-5">
        ...
        if ($_SESSION["user_category"] == "accountant") {
            $email = $_SESSION['user_email'];
            $accountant_query = "SELECT 
                miscellaneous_id, email, name
                FROM miscellaneous WHERE email = '$email'
                LIMIT 1
            ";
            $response = mysqli_query($conn, $accountant_query);
            $accountant_details = mysqli_fetch_array($response, MYSQLI_ASSOC);
            ...
            <div class='card account custom-shadow mt-5 p-2'>
                    ...
                    <form class='card-body' method='POST' action='./account-api.php?miscellaneous_id={$accountant_details['miscellaneous_id']}'>
                        <div class='form-group'>
                            <label>Full Name:</label>
                            <input type='text' class='form-control' name='name' value='{$accountant_details['name']}' required>
                        </div>
                    ...
                    <br>
                    <div class='text-center'>
                        <button type='submit' name='update_accountant' class='btn btn-outline-primary w-50'>Update Profile</button>
                    </div>

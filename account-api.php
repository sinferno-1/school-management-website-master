...
if (isset($_POST["update_accountant"])) {
    ...
    $find_accountant = "SELECT * FROM miscellaneous WHERE miscellaneous_id= '$miscellaneous_id'";
    $response = mysqli_query($conn, $find_accountant) or die(mysqli_error($conn));
    if (mysqli_num_rows($response) == 1) {
        if ($password == NULL) {
            $update_profile = "UPDATE 
                miscellaneous 
                SET name = '$name' 
                WHERE miscellaneous_id = '$miscellaneous_id'
            ";
        } else {
            $password = sha1($password);
            $update_profile = "UPDATE 
